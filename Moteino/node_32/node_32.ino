...
void setup(void)
{
  ...
  if(radio.initialize(FREQUENCY,NODEID,NETWORKID)){
    DEBUG(millis());DEBUG(":");DEBUGln("radio OK");
  } else {
    DEBUG(millis());DEBUG(":");DEBUGln("radio KO");
  }
  
#ifdef IS_RFM69HW
  radio.setHighPower(); //uncomment only for RFM69HW!
#endif
  //radio.setPowerLevel((uint8_t) 25);
  ...
  delay(15000);
  if(radio.sendWithRetry(GATEWAYID, "START", 6, 5, 8000)){
    DEBUGln("Start"); 
  } else {
    DEBUGln("Failed to send");
  }
...
char* BATstr="4.00";//"BAT:5.00v"; //longest battery voltage reading message = 9chars
byte sendLen;
char bufferT[10];
char bufferH[10];
...
void loop()
{
  ...
  if (sendLoops--<=0)   //send readings every SEND_LOOPS
  {
    ...
    sprintf(buffer, "BAT:%sv C:%s", BATstr, bufferT);
    
    //sprintf(buffer, "C:%s H:%s", bufferT, bufferH);
    sendLen = strlen(buffer);
    radio.sendWithRetry(GATEWAYID, buffer, sendLen, 2); //retry one time
